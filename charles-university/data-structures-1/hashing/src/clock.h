/**
 * Defines basic clock functionality
 * Code adapted from https://github.com/jbenet/nanotime
 *
 * @date 12/15/17
 * @author Dan Kondratyuk
 */

#ifndef HASHING_CLOCK_H
#define HASHING_CLOCK_H
...
struct nanotime nanotime_from_sec(uint64_t sec);
struct nanotime nanotime_from_sec_f(long double sec);
struct nanotime nanotime_from_usec(uint64_t usec);
struct nanotime nanotime_from_nsec(uint64_t nsec);
...
void nanotime_utc_offset_is(int64_t offset);
...
size_t nanotime_iso(const struct nanotime *nt, char *buf, size_t len);

#endif //HASHING_CLOCK_H
