...
class Mask(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        if type(inputs) is list:  # true label is provided with shape = [None, n_classes], i.e. one-hot code.
            ...
            x = keras.backend.sqrt(keras.backend.sum(keras.backend.square(inputs), -1))
            # generate the mask which is a one-hot code.
            # mask.shape=[None, n_classes]=[None, num_capsule]
            mask = keras.backend.one_hot(indices=keras.backend.argmax(x, 1),
                                            num_classes=x.get_shape().as_list()[1])

        # inputs.shape=[None, num_capsule, dim_capsule]
        # mask.shape=[None, num_capsule]
        # masked.shape=[None, num_capsule * dim_capsule]
        masked = keras.backend.batch_flatten(inputs * keras.backend.expand_dims(mask, -1))
        return masked
...
def squash(vectors, axis=-1):
    ...
    scale = s_squared_norm / (1 + s_squared_norm) / keras.backend.sqrt(s_squared_norm + keras.backend.epsilon())
...
class CapsuleLayer(keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        inputs_hat = keras.backend.map_fn(lambda x: keras.backend.batch_dot(x, self.W, [2, 3]),
                                             elems=inputs_tiled)

        # Begin: Routing algorithm ---------------------------------------------------------------------#
        # The prior for coupling coefficient, initialized as zeros.
        # b.shape = [None, self.num_capsule, self.input_num_capsule].
        b = tf.zeros(shape=[keras.backend.shape(inputs_hat)[0], self.num_capsule, self.input_num_capsule])
