    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            if args.rnn_cell == 'RNN':
                cell = tf.nn.rnn_cell.BasicRNNCell(args.rnn_cell_dim)
            elif args.rnn_cell == 'LSTM':
                cell = tf.nn.rnn_cell.BasicLSTMCell(args.rnn_cell_dim)
            else:
                # elif args.rnn_cell == 'GRU':
                cell = tf.nn.rnn_cell.GRUCell(args.rnn_cell_dim)

            state = cell.zero_state(1, tf.float32) # Create zero state using rnn_cell.zero_state call. Use batch size 1.
            predictions, loss = [], 0
            ...
            for i in range(self.TRAIN):
                ...
                input_ = self.sequence[i - 1] if i > 0 else 0.0
                input_ = tf.reshape(input_, [1, 1])

                x, state = cell(input_, state)
                x = dense(x)
                predictions.append(x)
                loss += tf.losses.mean_squared_error(tf.reshape(self.sequence[i], [1, 1]), x)
            ...
            for i in range(self.TEST):
                ...
                input_ = tf.reshape(predictions[-1], [1, 1])
                x, state = cell(input_, state)
                x = dense(x)
                predictions.append(x)

            # Generate `self.predictions` tensor (instead of Python list), use `tf.stack`.
            self.predictions = tf.reshape(tf.stack(predictions), [-1])
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        loss = network.prediction_summary(data, network.predict(data[:Network.TRAIN]))
