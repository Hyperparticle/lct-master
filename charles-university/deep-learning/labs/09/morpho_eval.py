...
with open(args.system, "r", encoding="utf-8") as system_file:
    system = [line.strip() for line in system_file if line.strip() != ""]

with open(args.gold, "r", encoding="utf-8") as gold_file:
    gold = [line.strip() for line in gold_file if line.strip() != ""]
...
print("Lemma accuracy: {:.5f}%".format(100 * same_lemma / total))
print("Tag accuracy: {:.5f}%".format(100 * same_tag / total))
